...
namespace vfh_local_planner
{
    ...
    void VFHPlanner::GetCandidateValleys()
    {
        ...
        for (int i=0; i < config_.vfh_sections_number; i++)
        {
            // std::cout<<vfh_histogram[i]<<std::endl;
        ...
        for(auto valley:candidate_valleys)
        {
            for(auto it:valley)
            {
                std::cout<<it<<" ";
        ...
        for (int k=0; k <= (config_.very_narrow_valley_threshold/2)-1; k++)
            {
                ...
                // std::cout<<vfh_histogram.size()<<std::endl;
                // std::cout<<vfh_histogram[upper_sector]<<" "<<vfh_histogram[lower_sector]<<std::endl;
                if (vfh_histogram[upper_sector] > config_.vfh_threshold || vfh_histogram[lower_sector] > config_.vfh_threshold)
                {
    ...
    double VFHPlanner::GetNewDirection(double global_plan_goal_direction, double current_robot_direction, double previews_direction)
    {
        ...
        for (int i=0; i < candidate_valleys.size(); i++)
        {
            ...
            // std::cout<<smallest_cost<<" "<<direction_cost<<" "<<best_valley<<std::endl;
            if (direction_cost < smallest_cost)
            {
                smallest_cost = direction_cost;
                best_valley = i;
        ...
        // std::cout<<best_valley<<std::endl;
        // std::cout<<candidate_valleys.at(best_valley).size()<<std::endl;
    ...
    bool VFHPlanner::RotateToGoal(const tf2::Stamped<tf2::Transform>& global_pose, const tf2::Stamped<tf2::Transform>& robot_vel, double goal_th, geometry_msgs::Twist& cmd_vel)
    {
        cmd_vel.linear.x = 0.08;

        double yaw = tf2::getYaw(global_pose.getRotation());
        double vel_yaw = tf2::getYaw(robot_vel.getRotation());
